...
local _G = _G
local assert = assert
local copcall = require"utils.coxpcall".copcall
local coxpcall = require"utils.coxpcall".coxpcall
local error = error
...
-- coxpcall call the err handler with the traceback as first argument (xpcall actually only gives the error m essage)
local function id(...)
    return ...
end
...
local function try_to_compile_and_run(self)
    ...
    local code, msg = loadstring(table.concat(self.lines),'@shell')
